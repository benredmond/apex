...
import { InvalidParamsError, ToolExecutionError, InternalError } from '../../../src/mcp/errors.js';
...
describe('PatternLookupService', () => {
  ...
  describe('Request Validation', () => {
    it('should reject requests without task', async () => {
      await expect(lookupService.lookup({})).rejects.toThrow(InvalidParamsError);
      expect(lookupMetrics.getMetrics().errors.validation).toBe(1);
    });
    
    it('should reject empty task', async () => {
      await expect(lookupService.lookup({ task: '' })).rejects.toThrow(InvalidParamsError);
    });
    
    it('should reject task exceeding max length', async () => {
      const longTask = 'a'.repeat(1001);
      await expect(lookupService.lookup({ task: longTask })).rejects.toThrow(InvalidParamsError);
    });
    
    it('should reject invalid max_size', async () => {
      await expect(lookupService.lookup({ 
        task: 'test',
  ...
  describe('Signal Extraction', () => {
    it('should extract language from current_file', async () => {
      ...
      expect(response.pattern_pack.task).toBe('Test task with typescript components');
  ...
  describe('Caching', () => {
    ...
    it('should use different cache keys for different max_size', async () => {
      ...
      const response1 = await lookupService.lookup({ ...baseRequest, max_size: 4096 });
      const response2 = await lookupService.lookup({ ...baseRequest, max_size: 8192 });
  ...
  describe('Rate Limiting', () => {
    it('should enforce rate limits', async () => {
      ...
      for (let i = 0; i < 101; i++) {
        promises.push(
          lookupService.lookup({ task: `Task ${i}` })
            .catch(e => e)
        );
      }
  ...
  describe('Error Handling', () => {
    it('should sanitize error messages', async () => {
      ...
      jest.spyOn(repository, 'lookup').mockRejectedValue(
