...
describe("PackBuilder", () => {
  ...
  describe("Size budget management", () => {
    ...
    it("should perform snippet trimming when over budget", async () => {
      ...
      // Snippet trimming may result in no snippet if budget is too small
      if (candidate.snippet) {
        const snippetLines = candidate.snippet.code.split("\n").length;
        expect(snippetLines).toBeLessThanOrEqual(18);
      } else {
        // If no snippet, verify it was due to budget constraints
        expect(result.pack.meta.bytes).toBeLessThanOrEqual(1000);
      }
    });
  });
  ...
  describe("Deduplication", () => {
    ...
    it("should not duplicate referenced patterns in separate lists", async () => {
      ...
      // These references may be moved to separate arrays in the pack
      const candidate = result.pack.candidates[0];
      const hasAntiRef =
        candidate.anti_refs?.includes("ANTI:SEC:1") ||
        result.pack.anti_patterns.some((p) => p.id === "ANTI:SEC:1");
      const hasTestRef =
        candidate.test_refs?.includes("TEST:UNIT:1") ||
        result.pack.tests.some((p) => p.id === "TEST:UNIT:1");

      expect(hasAntiRef).toBe(true);
