...
import ora from 'ora';
import { createPatternRepository, PatternRepository } from '../../storage/index.js';
...
export function createPatternsCommand(): Command {
  const patterns = new Command('patterns')
    .description('Manage APEX patterns');
  ...
  patterns
    ...
    .action(async () => {
      ...
      try {
        ...
        console.log(`  P50: ${searchTimes[Math.floor(iterations * 0.5)].toFixed(2)}ms`);
        console.log(`  P95: ${searchTimes[Math.floor(iterations * 0.95)].toFixed(2)}ms`);
        console.log(`  P99: ${searchTimes[Math.floor(iterations * 0.99)].toFixed(2)}ms`);
    ...
    .action(async (query: string, options: { limit: string }) => {
      try {
        ...
        for (const pattern of results) {
          ...
          console.log(chalk.gray(`  Trust: ${(pattern.trust_score * 100).toFixed(0)}%`));
          console.log();
        }
      } catch (error) {
