...
async function copyAgentTemplates(location) {
  const spinner = ora("Copying agent templates...").start();

  try {
    // [FIX:NODE:ESMODULE_IMPORTS] ★★★★★ - Proper ES module path resolution
    ...
    if (!(await fs.pathExists(agentTemplatePath))) {
      spinner.warn(chalk.yellow("⚠️  Agent templates not found in package"));
      return;
    }
    ...
    const templateFiles = await fs.readdir(agentTemplatePath);
    ...
    for (const file of mdFiles) {
      const sourcePath = path.join(agentTemplatePath, file);
    ...
    if (copiedCount > 0) {
      console.log(
        chalk.green(
          `✅ ${copiedCount} agent templates installed to ${targetPath}`,
        ),
      );
    }
    ...
    if (skippedCount > 0) {
      console.log(
        chalk.dim(`   (${skippedCount} existing templates preserved)`),
      );
    }

    if (copiedCount === 0 && skippedCount > 0) {
      console.log(
        chalk.yellow(`⚠️  All templates already exist at ${targetPath}`),
      );
    }
  } catch (error) {
    spinner.fail(chalk.red("Failed to copy agent templates"));
...
program
  ...
  .action(async () => {
    ...
    try {
      ...
      // Ask about agent templates
      const { installTemplates } = await inquirer.prompt([
        {
          type: "confirm",
          name: "installTemplates",
          message:
            "Would you like to install APEX agent templates for Claude Code?",
          default: true,
        },
      ]);
      ...
      if (installTemplates) {
        ...
        const { location } = await inquirer.prompt([
          {
            type: "list",
            name: "location",
            message: "Where would you like to install the templates?",
            choices: [
        ...
        // Copy templates
        await copyAgentTemplates(location);
      }
