...
export class BetaBernoulliTrustModel implements TrustModel {
  ...
  private calculateTrustFromParameters(alpha: number, beta: number): TrustScore {
    const value = alpha / (alpha + beta);
    const wilsonLower = this.calculateWilsonLowerBound(alpha, beta);
    const interval = this.calculateConfidenceInterval(alpha, beta);
    const confidence = 1 - (interval[1] - interval[0]);
    const samples = alpha + beta - this.config.defaultAlpha - this.config.defaultBeta;
  ...
  private calculateWilsonLowerBound(alpha: number, beta: number): number {
    ...
    if (n === 0 || (alpha === this.config.defaultAlpha && beta === this.config.defaultBeta)) {
      // Calculate from priors instead of magic number
      return this.config.defaultAlpha / (this.config.defaultAlpha + this.config.defaultBeta);
    }
  ...
  private betaIncompleteSeriesExpansion(x: number, a: number, b: number): number {
    const maxTerms = 200;
    let sum = 1.0;
    let term = 1.0;
    ...
    return (Math.pow(x, a) * Math.pow(1 - x, b) * sum) / (a * this.betaFunction(a, b));
  }
