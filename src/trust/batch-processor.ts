...
export class BatchProcessor {
    ...
    for (const [patternId, patternUpdates] of updateGroups) {
      try {
        ...
        for (const update of patternUpdates) {
          ...
          const daysSinceLastUpdate = (updateTime.getTime() - lastUpdated.getTime()) / (1000 * 60 * 60 * 24);
          
          if (daysSinceLastUpdate > 1) {
            // Get pattern type config for half-life
            const halfLife = this.getHalfLife(pattern.type, model);
            const decayFactor = Math.exp(-Math.LN2 * daysSinceLastUpdate / halfLife);
            
            // Decay towards priors
            const config = model.getConfig();
            alpha = config.defaultAlpha + (alpha - config.defaultAlpha) * decayFactor;
            beta = config.defaultBeta + (beta - config.defaultBeta) * decayFactor;
          }
      ...
      try {
        ...
        for (const [patternId, update] of updateMap) {
          try {
            ...
            errors.push({ patternId, error: `Batch update failed: ${errorMessage}` });
            failureCount++;
            successCount--;
          }
        }
   ...
   */
  private groupUpdatesByPattern(updates: TrustUpdate[]): Map<string, TrustUpdate[]> {
   ...
   */
  private getHalfLife(patternType: string, model: BetaBernoulliTrustModel): number {
    const config = model.getConfig();
