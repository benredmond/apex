...
function incompleteBetaRegularized(x: number, a: number, b: number): number {
  ...
  return (
    (Math.pow(x, a) * Math.pow(1 - x, b) * sum) / (a * Math.exp(logBeta(a, b)))
  );
}
...
export function generateASCIIVisualization(
  ...
  for (let row = height - 1; row >= 0; row--) {
    ...
    for (let col = 0; col < width; col++) {
      const point =
        distribution[Math.floor((col * distribution.length) / width)];
  ...
  // Add statistics
  plot.push(
    `Trust: ${trustScore.value.toFixed(3)} ` +
      `[${trustScore.interval[0].toFixed(3)}, ${trustScore.interval[1].toFixed(3)}]`,
  );
  plot.push(
    `Confidence: ${(trustScore.confidence * 100).toFixed(1)}% ` +
...
export function generateDecayVisualization(
  ...
  // Add key points
  plot.push(
    `Start: ${trustScore.value.toFixed(3)} → ` +
