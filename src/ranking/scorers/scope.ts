...
export function scoreScope(
  ...
  if (pattern.scope?.languages && signals.languages.length > 0) {
    const patternLangs = new Set(pattern.scope.languages.map(l => l.toLowerCase()));
  ...
  if (pattern.scope?.frameworks && (signals.frameworks.length > 0 || signals.deps)) {
    for (const patternFw of pattern.scope.frameworks) {
      ...
      for (const signalFw of signals.frameworks) {
        if (patternFw.name.toLowerCase() === signalFw.name.toLowerCase()) {
          if (patternFw.range && signalFw.version) {
            ...
            const satisfies = cached ?? semver.satisfies(signalFw.version, patternFw.range);
            if (cached === undefined) {
            ...
            if (satisfies) {
              ...
              frameworkMatch = frameworkMatch || `${patternFw.name} (version mismatch)`;
            }
          } else {
            frameworkPoints = Math.max(frameworkPoints, 8);
      ...
      if (signals.deps && signals.deps[patternFw.name]) {
        ...
        if (patternFw.range) {
          ...
          const satisfies = cached ?? semver.satisfies(version, patternFw.range);
          if (cached === undefined) {
            cache?.semver?.set(cacheKey, satisfies);
          }
