...
export class ParallelScorer {
  ...
  async scorePatterns(
    ...
    if (!this.isNode || candidateIndices.length < 100) {
      return this.scoreSingleThreaded(patterns, candidateIndices, signals, config);
    }
  ...
  private scoreOne(
    ...
    // Policy boost
    const policyPoints = pattern.type === 'POLICY' && 
                        (scopeResult.raw > 0 || !config.policyBoostRequiresScope) 
                        ? 20 : 0;
    
    const trustResult = scoreTrust(pattern, caches.wilson);
    const freshnessResult = scoreFreshness(pattern, Date.now(), config.halfLifeDefaultDays);
