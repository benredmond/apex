...
export function adaptStoragePattern(pattern: any): PatternMeta {
  ...
  const meta: PatternMeta = {
    ...
    scope: {
      ...
      frameworks: adaptFrameworks(
        fullPattern.scope?.frameworks || pattern.frameworks || [],
      ),
    },
    metadata: {
...
export async function createRankerFromRepository(
  ...
  return {
    rankPatterns: async (signals, k) => {
      ...
      const patterns: PatternMeta[] = result.patterns.map((p: any) =>
        adaptStoragePattern(p),
      );
