...
export class PatternDatabase {
  db;
  statements = new Map();
  constructor(dbPath = '.apex/patterns.db') {
  ...
  prepareStatements() {
    ...
    this.statements.set('getPattern', this.db.prepare('SELECT * FROM patterns WHERE id = ? AND invalid = 0'));
    ...
    `));
    this.statements.set('deletePattern', this.db.prepare('DELETE FROM patterns WHERE id = ?'));
