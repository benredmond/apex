...
export async function checkPatternWarnings(pattern: Pattern): Promise<string[]> {
  ...
  if (pattern.snippets) {
    ...
    if (totalLines > 200) {
      warnings.push(`Total snippet lines (${totalLines}) exceeds recommended limit of 200`);
    }
  }
  
  // Check for missing recommended fields
  if (pattern.type === 'ANTI' && (!pattern.evidence || pattern.evidence.length === 0)) {
    warnings.push('ANTI patterns should include evidence references');
  }
  
  if (pattern.type === 'FAILURE' && !pattern.signature && (!pattern.evidence || pattern.evidence.length === 0)) {
    warnings.push('FAILURE patterns should include either signature or evidence');
  }
  
  // Check trust score vs usage
  if (pattern.trust_score > 0.8 && (!pattern.usage || (pattern.usage.successes || 0) < 3)) {
    warnings.push('High trust score (>0.8) with low usage count (<3) is unusual');
  }
...
export async function validatePatternFile(filePath: string): Promise<ValidationResult> {
  try {
    // Read file content
