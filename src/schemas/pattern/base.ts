...
export const PatternIdSchema = z.string()
  .regex(/^[A-Z0-9_.-]+:[A-Z0-9_]+:[A-Z0-9_]+:[A-Z0-9_]+$/, 'Pattern ID must match format: ORG.TEAM:TYPE:CATEGORY:NAME');

export const TrustScoreSchema = z.number()
...
export const SnippetSchema = z.object({
  ...
  children: z.lazy(() => z.array(SnippetSchema).max(10, 'Maximum 10 nested snippets allowed').optional())
});
...
export const BasePatternSchema = z.object({
  ...
  // Identification
  id: PatternIdSchema,
  type: z.enum(['CODEBASE', 'LANG', 'ANTI', 'FAILURE', 'POLICY', 'TEST', 'MIGRATION']),
  ...
  semver_constraints: z.object({
    dependencies: z.record(z.string()).optional()
  }).optional(),
  
  // Content
  snippets: z.array(SnippetSchema).max(50, 'Maximum 50 snippets per pattern').optional(),
