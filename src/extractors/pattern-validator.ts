    ...
    for (const pattern of patterns) {
      try {
        ...
        if (this.seenHashes.has(hash)) {
          console.log(
            `[PatternValidator] Skipping duplicate pattern: ${pattern.title}`,
          );
          continue;
        }
        ...
        console.error(
          `[PatternValidator] Failed to validate pattern "${pattern.title}":`,
          error,
        );
      }
    }
    ...
    for (const line of lines) {
      ...
      if (
        ...
        if (!trimmed.endsWith(";") && !trimmed.endsWith(",")) {
          ...
          console.warn(
            `[PatternValidator] Possible missing semicolon: ${trimmed}`,
          );
        }
      }
    }
