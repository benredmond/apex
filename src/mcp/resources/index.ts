...
export class ResourceManager {
  ...
  getFile(id: string): FileResource {
    ...
    if (!isFileResource(resource)) {
      throw new InvalidResourceTypeError(
        `Expected file resource, got ${resource.type}`,
      );
    }
    return resource;
  }
  ...
  getPattern(id: string): PatternResource {
    ...
    if (!isPatternResource(resource)) {
      throw new InvalidResourceTypeError(
        `Expected pattern resource, got ${resource.type}`,
      );
    }
    return resource;
  }
  ...
  getBrief(id: string): BriefResource {
    ...
    if (!isBriefResource(resource)) {
      throw new InvalidResourceTypeError(
        `Expected brief resource, got ${resource.type}`,
      );
    }
    return resource;
  }
  ...
  createFile(
    ...
    name: string,
    content: string,
    additionalProps?: Partial<FileResource>,
  ): FileResource {
