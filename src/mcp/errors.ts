...
export class ResourceNotFoundError extends MCPError {
  constructor(resourceId: string, data?: unknown) {
    super(ErrorCode.ResourceNotFound, `Resource not found: ${resourceId}`, data);
...
export class ResourceAccessDeniedError extends MCPError {
  constructor(resourceId: string, data?: unknown) {
    super(ErrorCode.ResourceAccessDenied, `Access denied to resource: ${resourceId}`, data);
...
export class ToolExecutionError extends MCPError {
  constructor(toolName: string, error: string, data?: unknown) {
    super(ErrorCode.ToolExecutionError, `Tool execution failed for ${toolName}: ${error}`, data);
...
export class InvalidResourceTypeError extends MCPError {
  constructor(type: string, data?: unknown) {
    super(ErrorCode.InvalidResourceType, `Invalid resource type: ${type}`, data);
 ...
 */
export function isErrorResponse(response: unknown): response is { error: { code: number; message: string } } {
  return (
