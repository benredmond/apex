...
export class MigrationValidator {
   ...
   */
  private async verifyReversibility(
    db: Database.Database,
    migration: Migration,
  ): Promise<boolean> {
   ...
   */
  private async captureDbState(db: Database.Database): Promise<any> {
    const tables = db
      .prepare(
        `
   ...
   */
  private compareDbStates(before: any, after: any): boolean {
   ...
   */
  private async testSequentialApplication(
   ...
   */
  checkCommonIssues(migrations: Migration[]): string[] {
    const issues: string[] = [];
