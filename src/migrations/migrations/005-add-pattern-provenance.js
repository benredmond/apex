...
export const migration = {
    ...
    up: (db) => {
        ...
        db.transaction(() => {
            ...
            if (!columns.includes("provenance")) {
                ...
                db.exec("ALTER TABLE patterns ADD COLUMN provenance TEXT DEFAULT 'manual'");
            ...
            const updateResult = db
                .prepare("UPDATE patterns SET provenance = 'manual' WHERE provenance IS NULL")
                .run();
            console.log(`Updated ${updateResult?.changes || 0} existing patterns with provenance='manual'`);
    ...
    validate: (db) => {
        try {
            ...
            // Check index exists
            const indices = db
                .prepare("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_pattern_provenance'")
                .all();
            if (indices.length === 0) {
