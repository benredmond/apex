...
export const migration = {
  ...
  up: (db) => {
    ...
    db.transaction(() => {
      ...
      for (const pattern of patterns) {
        try {
          ...
          if (pattern.tags.trim().startsWith('[')) {
            console.log(`Pattern ${pattern.id} already uses JSON format, skipping`);
            skippedCount++;
            continue;
          }
          
          // Convert CSV to JSON array
          const csvTags = pattern.tags
            .split(',')
            .map(tag => tag.trim())
          ...
          console.log(`Migrated ${pattern.id}: "${pattern.tags}" -> ${jsonTags}`);
        } catch (error) {
      ...
      console.log(`Migration complete: ${migratedCount} patterns migrated, ${skippedCount} skipped`);
      ...
      if (metadataTableExists) {
        ...
        const metadataPatterns = db.prepare(`
        ...
        for (const meta of metadataPatterns) {
          try {
            ...
            console.error(`Failed to migrate metadata for ${meta.pattern_id}:`, error);
          }
        }
    ...
    db.transaction(() => {
      ...
      for (const pattern of patterns) {
        try {
          ...
          console.log(`Reverted ${pattern.id}: ${pattern.tags} -> "${csvTags}"`);
        } catch (error) {
      ...
      if (metadataTableExists) {
        ...
        for (const meta of metadataPatterns) {
          try {
            ...
            console.error(`Failed to revert metadata for ${meta.pattern_id}:`, error);
          }
        }
      }
    })();
      ...
      for (const pattern of patterns) {
        try {
          ...
          if (Array.isArray(tags)) {
            ...
            console.error(`Pattern ${pattern.id} has non-array JSON tags:`, pattern.tags);
            invalidCount++;
          }
        } catch (error) {
          console.error(`Pattern ${pattern.id} has invalid JSON tags:`, pattern.tags);
          invalidCount++;
        }
      }
