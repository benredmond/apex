...
export const migration = {
    ...
    up: (db) => {
        ...
        `);
                ...
                db.exec("CREATE INDEX IF NOT EXISTS idx_tasks_type_outcome ON tasks(task_type, outcome)");
                ...
                db.exec("CREATE INDEX IF NOT EXISTS idx_task_files_timestamp ON task_files(timestamp DESC)");
                // Similarity search indexes
                db.exec("CREATE INDEX IF NOT EXISTS idx_similarity_task_a ON task_similarity(task_a)");
                db.exec("CREATE INDEX IF NOT EXISTS idx_similarity_task_b ON task_similarity(task_b)");
                db.exec("CREATE INDEX IF NOT EXISTS idx_similarity_score ON task_similarity(similarity_score DESC)");
    ...
    validate: (db) => {
        try {
            ...
            // Insert test task
            db.prepare(`INSERT INTO tasks (id, title, task_type, status, phase) 
         VALUES ('TEST_001', 'Test Task', 'test', 'active', 'ARCHITECT')`).run();
            // Query test
            const result = db
            ...
            if (duration > 1500) {
                console.error(`Validation warning: Query performance ${duration}ms exceeds 1500ms target`);
            }
            console.log(`Migration 006 validation passed (performance: ${duration}ms)`);
            return true;
        }
        catch (error) {
