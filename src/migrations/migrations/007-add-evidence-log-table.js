...
export const migration = {
  ...
  validate: (db) => {
    try {
      ...
      db.prepare(
        `INSERT OR IGNORE INTO tasks (id, title, task_type, status) 
         VALUES ('TEST_EVIDENCE_001', 'Test Task', 'test', 'active')`,
      ).run();
      ...
      if (duration > 500) {
        console.error(
          `Validation warning: Query performance ${duration}ms exceeds 500ms target`,
        );
      }
      console.log(
        `Migration 007 validation passed (performance: ${duration}ms)`,
      );
      return true;
    } catch (error) {
