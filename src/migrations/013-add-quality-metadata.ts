...
export const migration: Migration = {
  ...
  up: (db: Database.Database) => {
    ...
    db.transaction(() => {
      ...
      db.exec(
        "CREATE INDEX IF NOT EXISTS idx_patterns_quality_score ON patterns(quality_score_cached)",
      );
      db.exec(
        "CREATE INDEX IF NOT EXISTS idx_patterns_last_activity ON patterns(last_activity_at)",
      );
      db.exec(
        "CREATE INDEX IF NOT EXISTS idx_patterns_quarantine ON patterns(quarantine_reason)",
      );
  ...
  down: (db: Database.Database) => {
    ...
    db.transaction(() => {
      ...
      console.log(
        "Note: Columns not removed (SQLite limitation). Indexes removed.",
      );
    })();
  },
};
