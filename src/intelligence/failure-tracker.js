...
export class FailureTracker {
  constructor(projectRoot = '.') {
    ...
    this.failuresPath = path.join(projectRoot, '.apex', '09_LEARNING', 'failures.jsonl');
    this.cache = null;
  ...
  suggestPrevention(failure) {
    const suggestions = {
      'ModuleNotFoundError': 'Check imports and ensure all dependencies are installed',
  ...
  async getStatistics() {
    ...
    // Count by type
    for (const failure of failures) {
      stats.byType[failure.error_type] = (stats.byType[failure.error_type] || 0) + 1;
      stats.byFile[failure.file_pattern] = (stats.byFile[failure.file_pattern] || 0) + 1;
    }
    
    // Find most common
    const sortedByOccurrence = [...failures].sort((a, b) => b.occurrences - a.occurrences);
